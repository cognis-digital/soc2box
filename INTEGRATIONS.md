...
**soc2box** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `soc2box` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
soc2box ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
soc2box ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
soc2box ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `soc2box` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="soc2box") for rec in records]   # records = your JSON output
